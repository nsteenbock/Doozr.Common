...
C:\projects\doozr-common\build\scripts\..\tools\OpenCover.4.6.519\tools\OpenCover.Console.exe -target:run_tests.cmd -filter:"+[Doozr.*]* -[*.Tests]*" -register:appveyor -searchdirs:..\..\src\Doozr.Common.Utilities\Doozr.Common.Utilities.Tests\bin\Debug -output:..\coverage\results.xml
